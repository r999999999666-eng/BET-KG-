...
def slots_start(msg):
    ...
    bot.reply_to(msg, 
        f"🎰 <b>Слоты</b>\n\n"
        f"{get_display_name(user)}, введи сумму ставки\n"
...
def slots_get_bet(msg):
    ...
    # Если игрок нажал другую кнопку — выходим
    if msg.text.lower() in ["кости", "монетка", "бонус", "баланс", "топ", "промокод",
                            "🎰 слоты", "🎲 кости", "🪙 монетка", "🎁 бонус", "💰 баланс", "🏆 топ", "🎟 промокод"]:
        return
    ...
    try:
        ...
        bot.reply_to(msg, "❌ Введи число. Пример: 5000")
        bot.register_next_step_handler(msg, slots_get_bet)
        return
    
    if bet < 50:
        bot.reply_to(msg, "❌ Минимальная ставка: 50")
        bot.register_next_step_handler(msg, slots_get_bet)
        return
    
    if bet > user['balance']:
        bot.reply_to(msg, f"❌ Недостаточно монет. Баланс: {user['balance']}")
        bot.register_next_step_handler(msg, slots_get_bet)
        return
